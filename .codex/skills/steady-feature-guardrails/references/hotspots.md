...
- `packages/app/components/plan-builder/RunStructureEditor.tsx` — `1518` lines. Large structured session editor shell. Structured template/materialization/volume-sync rules belong in `packages/app/features/plan-builder/structured-session-editor-engine.ts`; segment labels, target options, volume presets, and metric summaries belong in `packages/app/features/plan-builder/structured-session-editor-view-model.ts`; gesture-heavy row rendering belongs in `packages/app/components/plan-builder/RunStructureEditorParts.tsx`.
- `packages/app/components/plan-builder/SessionEditor.tsx` — `1532` lines. Large simple-session editor shell. Shared target/profile helpers belong in `packages/app/features/plan-builder/session-editor-targets.ts` or `packages/app/features/plan-builder/session-editing.ts`; simple save payload/materialization behavior belongs in `session-editing.ts`; target-pace section rendering belongs in `packages/app/components/plan-builder/SessionEditorTargetPaceSection.tsx`.
