...
export function formatNiggleSummary(
  niggle: Pick<Niggle, 'bodyPart' | 'bodyPartOtherText' | 'severity' | 'when' | 'side'>,
): string {
  const side = niggle.side ? `${titleCase(niggle.side)} ` : '';
  return `${side}${formatNiggleBodyPart(niggle)} · ${titleCase(niggle.severity)} · ${titleCase(niggle.when)}`;
}
