...
function rowToActivity(row: Record<string, unknown>): Activity {
  return {
    ...
    runSubtype: (row.run_subtype as Activity['runSubtype']) ?? undefined,
    ...
    avgCadence: row.avg_cadence != null ? Number(row.avg_cadence) : undefined,
...
function activityToRow(activity: Activity): Record<string, unknown> {
  const row: Record<string, unknown> = {
    ...
    source_name: activity.sourceName ?? null,
    source_device: activity.sourceDevice ?? null,
    run_subtype: activity.runSubtype ?? null,
